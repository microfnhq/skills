...
Claude Code skills for working with MicroFn - a cloud platform that turns JavaScript snippets into persistent, callable functions for your AI agent.
...
export async function main(input) {
  ...
  const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${input.city}`);
